...
document.addEventListener("DOMContentLoaded", () => {
  ...
  imageInput.addEventListener("change", async () => {
    ...
    try {
      ...
      alert(
        "Failed to upload image. Reason: " + (err.message || "Unknown error")
      );
    }
  });
  ...
  addBookForm.addEventListener("submit", async (e) => {
    ...
    if (!title || title.length < 5)
      return showAlert("Title must be at least 5 characters.", "danger");
    if (!author || author.length < 3)
      return showAlert("Author must be at least 3 characters.", "danger");
    if (isNaN(price) || price < 0)
      return showAlert("Price must be a valid positive number.", "danger");
    if (isNaN(stock) || stock < 0)
      return showAlert("Stock must be a valid positive number.", "danger");
    if (!description || description.length < 10)
      return showAlert("Description must be at least 10 characters.", "danger");
  ...
  function createBookRow(book) {
    ...
    row.innerHTML = `
        ...
        <td>${
          book.imageUrl
            ? `<img src="${book.imageUrl}" alt="${book.title}" style="max-width: 60px;">`
            : "None"
        }</td>
        <td>
            ...
            <button class="btn btn-sm btn-outline-danger delete-btn" data-id="${
              book.id
            }" data-title="${book.title}" title="Delete">
    ...
    tableBody.querySelectorAll(".delete-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        document.getElementById("delete-book-title").textContent =
          btn.dataset.title;
        document.getElementById("confirm-delete-book").dataset.id =
          btn.dataset.id;
        deleteModal.show();
      });
    });
  ...
  // Save Edited Book
  document
    .getElementById("save-book-changes")
    .addEventListener("click", async () => {
      const bookId = document.getElementById("edit-book-id").value;
      const title = document.getElementById("edit-book-title").value.trim();
      const author = document.getElementById("edit-book-author").value.trim();
      const category = document
        .getElementById("edit-book-category")
        .value.trim();
      ...
      const description = document
        .getElementById("edit-book-description")
        .value.trim();
      const imageUrl = document
        .getElementById("edit-book-image-url")
        .value.trim();

      // Validation edit modal
      if (!title || title.length < 5)
        return showAlertTable("Title must be at least 5 characters.", "danger");
      if (!author || author.length < 3)
